...
!!! warning "Nœuds tailnet RÉSIDENTS du LAN : `accept-routes` doit rester `false`"
    Un nœud Tailscale qui est **lui-même sur le LAN 192.168.1.0/24** (ex : sucre/LXC 105) et qui a `accept-routes: true` va router `192.168.1.28` via Tailscale au lieu du LAN direct. Conséquence : quand **penny initie** vers ce nœud (`ssh`, ping, health Homepage), le nœud **répond via Tailscale** → routage asymétrique → le `rp_filter` de penny droppe la réponse → 100% packet loss (sens penny→nœud uniquement). Vécu 2026-06-25 sur sucre. **Fix : `tailscale set --accept-routes=false`** sur les nœuds LAN-résidents (ils joignent penny en direct, ils n'ont pas besoin d'accepter la route). Diagnostic : `ip route get 192.168.1.28` sur le nœud doit montrer `dev eth0`, pas `dev tailscale0`. galahad/lancelot/dns-failover ne sont pas concernés (pas d'accept-routes).
