...
**tmux plutôt que `script(1)`.** La TUI de `claude` exige un pty ; tmux le fournit et rend la session attachable (`tmux -L claude attach -t claude`) pour voir son état exact. Bonus : la sortie ANSI reste dans le scrollback tmux au lieu d'inonder journald — on évite le chemin journald/mmap responsable des SIGBUS récurrents sur ARM. Socket dédié `-L claude` pour ne pas collider avec les sessions tmux interactives.
