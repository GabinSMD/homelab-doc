...
- [ ] Ce document en PDF
- [ ] **Un moyen de s'authentifier à GitHub** — voir l'encadré ci-dessous

:::danger[Dépendance circulaire — vérifié le 2026-08-26, non résolu]
`homelab-config` est un dépôt **privé**. Un `git clone` anonyme échoue :
...
Or c'est la **toute première commande de la phase 3**, et c'est elle qui va
chercher les configurations *et* les secrets chiffrés. La chaîne se referme sur
elle-même :

- pour cloner, il faut un identifiant GitHub ;
- l'identifiant est dans `.env.enc`… qui est **dans le dépôt** ;
- ou dans Vaultwarden… qui tourne sur le cluster, donc absent en perte totale.

La YubiKey déchiffre `restic-env.enc`, mais **seulement si on a le fichier**. En
perte totale, la procédure s'arrête à sa première ligne.

Deux façons de fermer la boucle, à trancher :

1. **Un PAT GitHub en lecture seule dans le coffre**, portée `repo` sur ces deux
   dépôts. Le plus simple, à faire tourner comme n'importe quel jeton.
2. **Une copie hors-ligne chiffrée du dépôt** sur la clé USB du coffre. Mesuré :
   **285 Mo** de contenu plus 22 Mo d'historique — ça tient partout. Avantage :
   aucune dépendance à un tiers pendant la reprise.

Tant que ce n'est pas fait, le scénario « perte totale » n'est pas exécutable,
même avec les deux YubiKeys.
:::
...
### Trouvé, et pas résolu

- **La dépendance circulaire GitHub** ci-dessus. C'est le seul point qui rend le
  scénario de perte totale non exécutable.
- **`restic` est en 0.14.0**, compilé en 2022. La série courante est bien plus
  récente. Ça fonctionne, mais un outil de reprise après sinistre est le dernier
  endroit où laisser vieillir une version.
